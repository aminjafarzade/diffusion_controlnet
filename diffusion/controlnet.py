...
# Import from local
from diffusion.unets.unet_2d_blocks import (
...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def from_unet(
        ...
        ls = ['conv_in', 'time_proj', 'time_embedding', 'down_blocks', 'mid_block']
        
        unet_dict = unet.state_dict()
        controlnet_dict = controlnet.state_dict()
        
        for name, weights in unet_dict.items():
            if name in ls:
                controlnet_dict[name].copy_(weights)

        controlnet.load_state_dict(controlnet_dict)
    ...
    def forward(
        ...
        temp_down_block_res_samples = (self.controlnet_down_blocks[i](sample) for i, sample in enumerate(down_block_res_samples))

        down_block_res_samples = temp_down_block_res_samples
